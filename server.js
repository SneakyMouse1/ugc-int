...
app.post('/api/contact', async (req, res) => {
  const { name, company, email, category, message, turnstileToken } = req.body;
  ...
  try {
    ...
    const subjectPrefix = category ? `[${category}] ` : '';
    const mailOptions = {
      ...
      subject: `New Contact Enquiry: ${subjectPrefix}from ${name} (${company || 'Individual'})`,
      text: `
Name: ${name}
Company: ${company || 'N/A'}
Email: ${email}
...
${category ? `<p><strong>Trade / Logistics Scope:</strong> ${category}</p>` : ''}
<br/>
